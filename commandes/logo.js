...
zokou({ nomCom: "hacker",
    ...
    try {
      ...
      //  let lien = "https://e1.yotools.net" + res;
        repondre("* processing...*");
        await zk.sendMessage(origineMessage, { image: { url:anu.image}, caption: "* \t 𝚃𝚑𝚒𝚜 𝙻𝚘𝚐𝚘 𝚠𝚊𝚜 𝚐𝚎𝚗𝚎𝚛𝚊𝚝𝚎𝚍 𝚋𝚢 𝙿𝚑𝚊𝚗𝚝𝚘𝚖" }, { quoted: ms });
    }
    catch (e) {
        repondre("👑 " + e);
    }
...
zokou({ nomCom: "dragonball", categorie: "Logo", reaction: "🐉" }, async (dest, zk, commandeOptions) => {
    ...
    try {
       ...
       // var idImg = Object.values(imgInfo)[3];
        ...
        await zk.sendMessage(dest, { image: { url: imgInfo.image }, caption: "* \t Logo by BONIPHACE-MD*" }, { quoted: ms });
    }
    catch (e) {
        repondre(",👑 " + e);
    }
...
zokou({ nomCom: "naruto", categorie: "Logo", reaction: "⛩" }, async (dest, zk, commandeOptions) => {
    ...
    try {
        ...
        await zk.sendMessage(dest, { image: { url: img.image }, caption: "\t\t *Logo by BONIPHACE-MD*" }, { quoted: ms });
    }
    catch (e) {
        repondre("👑 " + e);
    }
...
zokou({nomCom : "wall",categorie : "Logo",reaction : "👍"} , async (dest,zk,commandeOptions) =>{
  ...
  if(!arg[0]) { repondre( `𝙴𝚡𝚊𝚖𝚙𝚕𝚎:\n ${prefixe}𝚠𝚊𝚕𝚕 𝙿𝚑𝚊𝚗𝚝𝚘𝚖` ); return ;}
   let text = arg.join(" ")
...
zokou({nomCom: "summer", categorie: "Logo", reaction: "🌞"}, async (dest, zk, commandeOptions) => {
  ...
  mumaker.textpro("https://textpro.me/create-sunset-light-text-effects-online-for-free-1124.html", text)
    .then((data) => {
      zk.sendMessage(dest, { image: { url: data.image }, caption: '𝚕𝚘𝚐𝚘 𝚋𝚢 𝙿𝚑𝚊𝚗𝚝𝚘𝚖' }, { quoted: ms });
    })
    .catch(console.error);
});
...
zokou({nomCom: "neonlight", categorie: "Logo", reaction: "💡"}, async (dest, zk, commandeOptions) => {
  ...
  try{
    ...
    /*.then((data) => {*/
     zk.sendMessage(dest, { image: { url: maker.image }, caption: '𝙻𝚘𝚐𝚘 𝚋𝚢' }, { quoted: ms });
...
zokou({nomCom: "greenneon", categorie: "Logo", reaction: "🟢"}, async (dest, zk, commandeOptions) => {
  ...
  mumaker.textpro("https://textpro.me/green-neon-text-effect-874.html", text)
    .then((data) => {
      zk.sendMessage(dest, { image: { url: data.image }, caption: '𝙻𝚘𝚐𝚘 𝚋𝚢' }, { quoted: ms });
    })
    .catch(console.error);
});
  ...
  {nomCom:"boom",categorie:
    ...
    let {ms,repondre,prefixe,arg}=commandeOptions;
        ...
        if(!arg||arg=="")
        {
          ...
          repondre(`  𝙴𝚡𝚊𝚖𝚙𝚕𝚎:* ${prefixe}𝚋𝚘𝚘𝚖 𝚉𝚘𝚔𝚘𝚞`)
          return ;
        }
       try{
              var radio2 = "e0723d60-fc0d-421f-bf8f-a9b9b61e4be6";
...
repondre("  processing ...")
         await zk.sendMessage(dest,{image:{url:img.image},caption:"\t *𝙻𝚘𝚐𝚘 𝚋𝚢 𝙿𝚑𝚊𝚗𝚝𝚘𝚖*"})
         
        // await zk.sendMessage(dest,{image:{url:lienImage},caption:"\t *Logo by Boniphace-Md*"})
...
zokou({nomCom:"gold",categorie:"Logo",reaction:"🧚🏿‍♀️"},async(dest,zk,commandeOptions)=>{
  ...
  try{
    ...
    var img = await mumaker.ephoto(lien,arg);
   repondre("processing ...")
    await zk.sendMessage(dest,{image:{url:img.image},caption:" *𝙻𝚘𝚐𝚘 𝚋𝚢 𝙿𝚑𝚊𝚗𝚝𝚘𝚖*"},{quoted:ms})
  }catch(e){repondre(e)}
})
...
zokou({ nomCom: "gif×1", categorie: "Logo", reaction: "😋" }, async (dest, zk, commandeOptions) => {
  ...
  try {
    if (!arg || arg === "") {
      repondre(`${prefixe}𝚐𝚒𝚏×1 𝙿𝚑𝚊𝚗𝚝𝚘𝚖`);
      return;
    }
    ...
    repondre("Processing...");
    await zk.sendMessage(dest, { image: { url: img }, caption: "*𝙻𝚘𝚐𝚘 𝚋𝚢 𝙿𝚑𝚊𝚗𝚝𝚘𝚖*" }, { quoted: ms });
  } catch (e) {
    repondre(e);
  }
});
